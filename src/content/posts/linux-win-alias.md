...
updated: 2026-03-09
description: 在 Bash 和 Windows PowerShell 中设置命令别名，并写入配置文件以便长期使用。
category: 系统与工具
tags:
  - Linux
  - Windows
...
<span id="🐧-linux-给当前用户添加别名-ll" class="legacy-anchor" aria-hidden="true"></span>
...
<span id="🌍-linux-给所有用户添加别名" class="legacy-anchor" aria-hidden="true"></span>

## 🌍 Linux 给所有用户添加别名

上述操作仅对当前 SSH 用户生效。如果想要对服务器上的所有用户生效，对于 `bash` 用户而言，打开 `/blog/bash.bashrc`，重复上述操作即可。

<span id="🪟-windows-powershell-添加别名" class="legacy-anchor" aria-hidden="true"></span>
